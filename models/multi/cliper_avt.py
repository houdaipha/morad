...
class CLIPer(nn.Module):
    def __init__(self, config, device='cpu'):
        ...
        self.temporal_out = nn.Linear(
            5 * config.temporal_head.out_features,
            config.out_features)
    ...
    def forward(self, x, y, z, mask=None):
        ...
        xy = self.temporal_head_av(xy)

        yz = torch.cat([y, z], dim=-1)
        yz = self.temporal_head_at(yz)

        zo = torch.cat([xo, xy, yo, yz, zo], dim=-1)
...
def main():
    ...
    with open('configs/multi/clipere_tt.yaml', 'r') as f:
    ...
    summary(cliper, input_data=[x, y, z])
