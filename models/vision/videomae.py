...
# SOURCE: https://github.com/MCG-NJU/VideoMAE
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv3d(
            in_channels=in_chans, out_channels=embed_dim, kernel_size=(
                self.tubelet_size, patch_size[0], patch_size[1]), stride=(
                self.tubelet_size, patch_size[0], patch_size[1]))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 # Note: Added by me, Seems like different versions of timm !
                 pretrained_cfg=None,
...
def load_state_dict(
    ...
    if len(ignore_missing_keys) > 0:
        logger.info("Ignored weights of {} not initialized from pretrained model: {}".format(
            model.__class__.__name__, ignore_missing_keys))
    if len(error_msgs) > 0:
        logger.info('\n'.join(error_msgs))


def load_checkpoint(model, checkpoint_model, num_frames=32, prefix=''):
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        if orig_size != new_size:
            logger.info("Position interpolate from %dx%d to %dx%d" %
            ...
            pos_tokens = pos_tokens.permute(0,
                                            2,
                                            3,
                                            1).reshape(-1,
                                                       num_frames // model.patch_embed.tubelet_size,
    ...
    load_state_dict(model, checkpoint_model, prefix=prefix)
...
class VideoMAE(nn.Module):
    def __init__(self, config, device='cpu'):
        ...
        if self.config.out_features != self.videomae.num_classes:
            self.videomae.reset_classifier(self.config.out_features)
    ...
    def _init_videomae(self):
        ...
        depth = len([k for k in state_dict.keys() if k.endswith('attn.proj.weight')])
        heads = embed_dim // 64
        mlp_ratio = state_dict['blocks.0.mlp.fc1.weight'].shape[0] // state_dict['blocks.0.mlp.fc1.weight'].shape[1]
        qkv_bias = 'blocks.0.attn.q_bias' in state_dict
        ...
        load_checkpoint(model, state_dict, num_frames=self.config.frames)
...
def main():
    ...
    with open('configs/videomae.yaml', 'r') as f:
        config = yaml.safe_load(f)

    config = VideoMAEConfig(**config['model'])
